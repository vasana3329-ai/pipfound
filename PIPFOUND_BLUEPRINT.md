...
| `wiring_check_test.py` | ~215 | **جهش‌آزماییِ چکِ «اتصالِ HTML و JS» (لایهٔ ۱).** ۷۸ بررسیِ آفلاینِ قطعی روی کپیِ موقتِ `app.py`: جهش‌های واقعی (idِ تکراری، ارجاعِ JS به idِ ساخته‌نشده، هندلرِ inline، `x.onclick = نامِ ناموجود`، `addEventListener`/تایمر با نامِ خالی، همه در هر دو سند) باید گیت را قرمز کنند و **نامِ همان مورد** را بگویند؛ جهش‌های بی‌گناه (مارک‌آپِ ساخته‌شده داخلِ رشتهٔ JS، idِ کامنت‌شده، هندلرِ فلش/متد، کلاسی که فقط JS/CSS استفاده‌اش می‌کند، idِ همنام در سندِ دیگر) نباید قرمز کنند؛ و «استفاده‌نشده»ها فقط هشدار بمانند. در CI بلافاصله بعد از `undefined_call_test.py`. |
| `ui_visual_check.cjs` | ~683 | **تستِ بصری/رفتاری روی کرومِ headless.** در CI لایهٔ ۳: اجرا شدنِ اسکریپت، نبودِ خطای کنسول/زمانِ اجرا، حضور و سیم‌کشیِ کلیدها، و رفتارهای واقعی (چیپِ نماد، سبک، «↻ بروزرسانی»، ریسک) — و **بندِ ۶.۷: قراردادِ نشانگر روی کرکره‌ی نمادها** (پنل روی هیچ کنترلِ ردیفِ جستجو ننشیند، با رفتنِ نشانگر روی کرکره بسته نشود، و با انتخاب/دورشدنِ واقعی بسته شود). |
