...
| `(این دور)` | **کلیدِ 📰 فاندمنتال (خواسته‌ی کاربر):** ماژولِ جدیدِ `fundamental.py` روی `macro_context.get_calendar()` سوار است (بدونِ منبعِ جدید، بدونِ کلید). هر خبرِ High/Medium را دسته‌بندی می‌کند (نرخِ بهره/تورم/رشد/اشتغال/PMI/…) و برای دو سناریوی «بهتر از انتظار» و «بدتر از انتظار» جهتِ اثر روی جفت‌ارزهای مهم + طلا و نقره را می‌دهد؛ با منطقِ درستِ فاندمنتال: دیتای قویِ آمریکا → دلار قوی → طلا/نقره پایین؛ نرخِ بیکاری/مدعیانِ بیکاری معکوس؛ اخبارِ غیردلاری اثرِ حاشیه‌ای روی طلا. زمان به دو وقتِ نیویورک و تهران + شمارشِ معکوسِ فارسی. اندپوینت `/api/fundamental?hours=N` و صفحه‌ی مجزای `/fundamental` (تبِ نو). دکمه‌ی `#fundBtn` (گرادیانِ آبی/بنفش) کنارِ سایرِ دکمه‌ها. **آلارم:** `_fund_alarm_worker` هر ۱۰ دقیقه، ~۲۴ ساعت پیش از هر خبرِ پرتأثیر یک نوتیفیکیشنِ نیتیوِ مک می‌فرستد (یک‌بار، با dedupe در `fund_alarms_fired.json`). |
