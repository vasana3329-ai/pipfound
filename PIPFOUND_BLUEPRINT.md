...
- **S2 — ⏳ OPEN (یافتهٔ ۲۰۲۶-۰۹-۲۴). نگهبان «صدا‌زده‌شده ولی تعریف‌نشده» را نمی‌بیند:** لایهٔ ۱ سینتکسِ JS و «هیچ کلیدی گم نشود» را می‌سنجد، ولی اگر تابعی (مثلِ `closePick`/`closePickSoon`) در چند نقطهٔ صفحه صدا زده شود و تعریفش حذف شده باشد، `node --check` ساکت است و همه‌ی آن مسیرها در زمانِ اجرا با `ReferenceError` بی‌صدا می‌میرند — همین یک‌بار چند نوبت «پنل هیچ‌وقت نمی‌بندد» را پنهان کرد و کسی نفهمید. راهِ بستن: یک چکِ سبکِ استاتیک روی بلوکِ `<script>` (فهرستِ شناسه‌های صدا‌زده‌شده در برابر فهرستِ تعریف‌ها، با allowlistِ APIهای مرورگر) به‌عنوان بخشی از لایهٔ ۱.
