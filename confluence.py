...
def score(symbol, tfs, d=None):
    ...
    # --- 5b. توالیِ sweep → MSS (فیکس C4) ---
    # ترتیبِ مقدسِ ICT: اول سوئیپِ لیکوئیدیتی، بعد شکستِ ساختار (MSS)، بعد ورود.
    # smc_engine.sequence_ok روی LTF این توالی را می‌سنجد. اگر MSS قبل از سوئیپ
    # رخ داده باشد (توالیِ نقض‌شده)، این یک امتیازِ منفی/گیت است.
    seq_ok = d.get(ltf, {}).get("sequence_ok")
    if seq_ok is True:
        row("توالیِ سوئیپ→ام‌اس‌اس", True, 1.0,
            "ترتیبِ درست: سوئیپِ لیکوئیدیتی پیش از شکستِ ساختار (ام‌اس‌اس)")
    elif seq_ok is False:
        row("توالیِ سوئیپ→ام‌اس‌اس", False, 1.0,
            "توالیِ نقض‌شده: شکستِ ساختار پیش از سوئیپ رخ داده — ورود پرریسک")
    else:
        row("توالیِ سوئیپ→ام‌اس‌اس", None, 1.0,
            "سوئیپ یا ام‌اس‌اسِ واضحی برای سنجشِ توالی نیست")
